...
S3ConcurrentListObjectStream.prototype.handleError = function (error) {
  if (error) {
      ...
      function (error, nextContinuationToken, s3Objects, commonPrefixes) {
        ...
        if (firstCall) {
          ...
          _.each(commonPrefixes, function (commonPrefix) {
            ...
            self.queue.push(subDirectoryOptions, self.handleError.bind(self));
          });
        }
...
S3ConcurrentListObjectStream.prototype.processIncomingObject = function (
  ...
  this.queue.push(options, self.handleError.bind(self));
};
