 ...
 * @param {Function} callback - Callback of the form
    function (error, nextMarker, String[]).
 */
S3ConcurrentListObjectStream.prototype.listCommonPrefixesPage = function (
  ...
  async.retry(
    ...
    function (error, response) {
      ...
      callback(null, nextMarker, response.CommonPrefixes);
    }
  );
};
...
S3ConcurrentListObjectStream.prototype.listCommonPrefixes = function (
  ...
  function listRecusively (marker) {
    ...
    self.listCommonPrefixesPage(
      options,
      function (error, nextMarker, commonPrefixesPage) {
        if (error) {
          return callback(error);
        }
...
S3ConcurrentListObjectStream.prototype.listObjectsConcurrently = function (
  ...
  var tasks = _.map(options.commonPrefixes, function (commonPrefix) {
    return function (asyncCallback) {
      ...
      // Common prefixes returned by the AWS API will be postfixed by the
      // delimiter. Prefixes passed in from other sources may not be.
      //
      // Sort out the prefix situation. If the general prefix is 'a/b', and
      // commonPrefix for this task is 'c/d/', then the prefix passed in to the
      // list method should be 'a/b/c/d/'.
      if (taskOptions.prefix) {
        if (!taskOptions.prefix.endsWith(taskOptions.delimiter)) {
          taskOptions.prefix += taskOptions.delimiter;
        }

        taskOptions.prefix += commonPrefix;
      }
      // Or if no general prefix, then just use the commonPrefix.
      else {
        taskOptions.prefix = commonPrefix;
      }
